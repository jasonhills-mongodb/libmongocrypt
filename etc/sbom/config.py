...
def run_repo_version_script() -> str:
    ...
    try:
        # VERSION_CURRENT file
        file_path = Path("VERSION_CURRENT")
        ...
        version = semver.VersionInfo.parse(content).finalize_version()
        ...
        logger.warning(
            'PRIMARY COMPONENT VERSION: Unable to parse output from VERSION_CURRENT or etc/calc_release_version.py: %s', result.stdout
        )
        logger.warning(e)
        return ""
